...
def load_stimuli() -> pd.DataFrame:
        ...
        df = pd.DataFrame([
            {
                "item_id": 1,
                "scenario": "朋友考研失败后很沮丧",
                "text": "这次没有上岸一定很难受，但这不等于你不行。先让自己缓一缓，等情绪过去一点，我们再一起看哪里还能调整。",
                "true_source": "human",
            },
            {
                "item_id": 2,
                "scenario": "朋友考研失败后很沮丧",
                "text": "你现在感到失落是完全可以理解的。一次结果不能定义你的价值，你已经很努力了，休息一下之后我们可以一起梳理下一步。",
                "true_source": "ai",
            },
            {
                "item_id": 3,
                "scenario": "同学被导师批评后怀疑自己",
                "text": "被导师批评真的会让人一下子泄气，但被指出问题不代表你没有潜力。很多时候，这只是成长里很刺耳的一步。",
                "true_source": "human",
            },
            {
                "item_id": 4,
                "scenario": "同学被导师批评后怀疑自己",
                "text": "我能理解你现在的受挫感。导师的反馈虽然刺耳，但通常针对的是当前表现，而不是你这个人本身。你并不是没有能力。",
                "true_source": "ai",
            },
            {
                "item_id": 5,
                "scenario": "朋友失恋后整晚睡不着",
                "text": "失恋最难熬的就是晚上，脑子停不下来。你不用逼自己马上放下，先撑过这几天就已经很不容易了。",
                "true_source": "human",
            },
            {
                "item_id": 6,
                "scenario": "朋友失恋后整晚睡不着",
                "text": "你现在反复想起这段关系是很正常的。不要要求自己立刻恢复，情绪需要时间消化，你已经在经历一个不容易的过程。",
                "true_source": "ai",
            },
        ])
        df.to_csv(STIMULI_PATH, index=False, encoding="utf-8-sig")
    ...
    df["true_source"] = df["true_source"].str.lower().str.strip()
    return df
...
def save_responses(rows: list[dict]) -> None:
    ...
    if RESPONSES_PATH.exists() and RESPONSES_PATH.stat().st_size > 0:
        df_old = pd.read_csv(RESPONSES_PATH)
